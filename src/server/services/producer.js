const producer = {
  ...
  gameGeneral: (res) => {
    ...
    res['sports_content']['games']['game'].forEach((game, index) => {
      ...
      sides.forEach(key => {
        ...
        item[key]['team'] = game[key]['abbreviation'] + '.' + game[key]['nickname']
  ...
  gameDetail: (res) => {
    ...
    Object.keys(result).forEach(side => {
      ...
      result[side].linescores = data[side].linescores
      result[side].Leaders = data[side].Leaders
      result[side].stats = data[side].stats
   ...
   */
  teamList: (eastTeams, westTeams) => {
    const eastData = eastTeams.resultSets[0].rowSet
    const westData = westTeams.resultSets[0].rowSet
    var eastern = []
    var western = []
    ...
    eastern.sort((a, b) => { return (b.win - a.win) })
    western.sort((a, b) => { return (b.win - a.win) })
